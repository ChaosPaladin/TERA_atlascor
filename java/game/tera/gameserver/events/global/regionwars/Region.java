...
public class Region extends SafeTask implements DieListener, PlayerSelectListener
{
	...
	private synchronized void register(Npc npc, Player player)
	{
		...
		if(guildRank == null)
		{
			player.sendMessage("You are not a member of a guild.");
			return;
		}
		...
		{
			player.sendMessage("You are not a guildmaster.");
			return;
		}
		...
		if(guild == getOwner())
		{
			player.sendMessage("The guild is the owner of the region.");
			return;
		}
		...
		{
			player.sendMessage("You are already registered in another battle.");
			return;
		}
		...
		if(!dbManager.insertRegionGuildRegister(this, guild))
			player.sendMessage("There was an error, please contact Tech Support.");
		else
		{
			...
			if(event.isOwnerGuild(guild))
				player.sendMessage("You have successfully registered for the battle, ownership of your region will be canceled at the beginning of the battle.");
			else
				// сообщаем о успешной регистрации
				player.sendMessage("You have successfully registered for the battle.");
		}
	}
	...
	private synchronized void unregister(Npc npc, Player player)
	{
		...
		if(!event.isRegisterGuild(guild))
		{
			player.sendMessage("You are not registered in any battle.");
			return;
		}
		...
		{
			player.sendMessage("You have not been registered at the battle.");
			return;
		}
		...
		// сообщаем об успехе
		player.sendMessage("Your guild has been successfully battle with otregestrirovana.");
	}
