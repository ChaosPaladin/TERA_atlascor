	...
	public void execution(String command, Player player, String values)
	{
		switch(command)
		{
			case "chat_ban":
			{
				...
				World.sendAnnounce(player.getName() + " blocked Chat Players " + name + " to " + stringDate + " reason: " + comment);
			...
			case "chat_unban":
			{
				...
				{
					player.sendMessage("Player chat is unblocked.");
					return;
				}

				target.setEndChatBan(0);

				World.sendAnnounce(player.getName() + " Players chat unblocked " + values);
			}
		}
	}
}
