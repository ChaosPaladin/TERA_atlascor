...
public class GuildCreateAction extends AbstractAction<String>
{
	...
	public synchronized void assent(Player player)
	{
		...
		if(party == null)
		{
			...
			player.sendMessage("You must be in a group.");
			return;
		}
	...
	public boolean test(Player actor, String target)
	{
		...
		if(actor.getLevel() < 8)
		{
			actor.sendMessage("You have insufficient.");
			return false;
		}
		...
		{
			// сообщаем об этом и выходим
			actor.sendMessage("You must be in a group.");
			return false;
		}
		...
		try
		{
			...
			for(int i = 0, length = members.size(); i < length; i++)
				...
				{
					// сообщаем и выходим
					actor.sendMessage("There is a player in your group that is in a guild.");
					return false;
				}
		}
		finally
		...
		{
			// сообщаем и выходим
			actor.sendMessage("You do not have the money.");
			return false;
		}
