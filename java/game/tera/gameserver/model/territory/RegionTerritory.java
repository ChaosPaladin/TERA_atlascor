...
public class RegionTerritory extends AbstractTerritory
{
	...
	public void onEnter(TObject object)
	{
		...
		// если его нет
		if(owner == null)
			player.sendMessage("You entered the neutral region.");
		...
		{
			player.sendMessage("You entered the region belongs \"" + owner.getName() + "\".");
			region.addFuncsTo(player);
		}
	}
	...
	public void onExit(TObject object)
	{
		...
		{
			player.sendMessage("You came from the region belonging \"" + owner.getName() + "\".");
			region.removeFuncsTo(player);
		}
	}
}
