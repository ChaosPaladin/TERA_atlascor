...
public abstract class AbstractAutoEvent extends SafeTask implements Event, Synchronized, NpcInteractEvent, Registered, TerritoryListener, DieListener, DeleteListener {
	...
	public boolean registerPlayer(Player player) {
		...
		try {
			...
			if(getState() != EventState.REGISTER) {
				player.sendMessage("Registration time left.");
				return false;
			}

			if(player.getLevel() > getMaxLevel() || player.getLevel() < getMinLevel()) {
				player.sendMessage("Not enough level.");
				return false;
			}
			...
			if(prepare.contains(player)) {
				player.sendMessage("You already are registered.");
				return false;
			}
	...
	protected void registerState() {
		...
		World.sendAnnounce("Event will start in " + time + " minutes(а)");

		if(isNeedShowPlayerCount()) {
			World.sendAnnounce("Registered " + prepare.size() + " member(ов)");
		}

		World.sendAnnounce("Register NPC [Mystel] or through the command --event_reg " + getName());
	...
	public boolean start() {
		...
		try {
			...
			World.sendAnnounce("Event Launched \"" + getName() + "\"");
	...
	public boolean unregisterPlayer(Player player) {
		...
		try {
			...
			if(!isStarted()) {
				player.sendMessage("Event is not open.");
				return false;
			}
