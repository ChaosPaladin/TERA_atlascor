...
public class JoinGuildAction extends SafeTask implements Action, Foldable
{
	...
	public synchronized void assent(Player player)
	{
		...
		try
		{
			...
			if(actor.hasGuild())
			{
				player.sendMessage("You already are a member of the guild.");
				return;
			}
			...
			if(party == null)
			{
				player.sendMessage("You have to be in a group.");
				return;
			}
			...
			try
			{
				...
				for(int i = 0, length = members.size(); i < length; i++)
					if(array[i].hasGuild())
					{
						actor.sendMessage("In your group already has a man in a guild.");
						return;
					}
			}
			finally
	...
	public synchronized void cancel(Player palyer)
	{
		...
		try
		{
			...
			for(int i = 0, length = members.size(); i < length; i++)
				array[i].sendMessage("Create clan interrupted due to non consent of all members of the group.");

			actor.sendMessage("Create clan interrupted due to non consent of all members of the group.");
		}
		finally
		{
			members.readUnlock();
