...
public class UserCommand extends AbstractCommand {
	...
	public UserCommand(int access, String[] commands) {
		...
		help = Files.read(Config.SERVER_DIR + "/notes/help.txt");
	}
		...
		switch(command) {
			...
			case "end_pay": {
				...
				if(System.currentTimeMillis() > time)
					...
					synchronized(date) {
						date.setTime(time);
						player.sendMessage("Expiration: " + timeFormat.format(date));
					}
				}
			...
			case "version":
				player.sendMessage("Server Version: " + Config.SERVER_VERSION);
				break;
			case "online": {
				OnlineManager onlineManager = OnlineManager.getInstance();

				player.sendMessage("Online Players: " + onlineManager.getCurrentOnline());
