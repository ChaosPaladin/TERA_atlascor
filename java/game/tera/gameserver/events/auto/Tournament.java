...
public class Tournament extends AbstractAutoEvent
{
	...
	protected void finishingState()
	{
		...
		if(teams.size() != 1)
			World.sendAnnounce("The winner is not ...");
		else if(teams.size() == 1)
		{
			...
			// сообщаем об этом
			World.sendAnnounce("The winning team \"" + team.getName() + "\".");
			...
			for(int i = 0, length = team.size(); i < length; i++)
			{
				...
				// сообщаем об награде
				player.sendMessage("You got " + reward + " point(s) of Fame.");
			}
		}
	...
	protected void prepareBattleState()
	{
		...
		// сообщаем о бое
		World.sendAnnounce("Phase " + (level + 1) + " : bout 1 minute \"" + first.getName() + "\" Vs \"" + second.getName() + "\"!");
	...
	protected void prepareEndState()
	{
		...
		{
			// сообщаем об этом
			World.sendAnnounce("The winning draw...");
		...
		else
		{
			...
			// сообщаем об этом
			World.sendAnnounce("The winning team \"" + winner.getName() + "\"!!!");
	...
	protected void prepareStartState()
	{
		...
		World.sendAnnounce("The event will involve " + players.size() + " player(s) and " + teams.size() + " commands(а).");
