		...
		switch(index) {
			case SOURCE_ITEM_INDEX: {
				...
				if(getConsume() != null || getAlkahest() != null) {
					actor.sendMessage("It is necessary to clean the used items and alkshest.");
					return;
				}
				...
				if(source == null) {
					actor.sendMessage("Cannot find anything.");
					return;
				}

				if(source.getEnchantLevel() >= MAX_ENCHANT_LEVEL) {
					actor.sendMessage("Enchant level already max.");
					return;
				}

				if(!source.isEnchantable()) {
					actor.sayMessage("Cant enchant this item.");
					return;
				}
			...
			case CONSUME_ITEM_INDEX: {
				...
				if(source == null) {
					actor.sendMessage("Unknown item crafting.");
					return;
				}
				...
				if(source.getExtractable() != consume.getExtractable()) {
					actor.sendMessage("This item is not suitable for the level of enchantment.");
					return;
				}

				if(!source.getClass().isInstance(consume)) {
					actor.sendMessage("This item is not suitable type.");
					return;
				}
			...
			case ALKAHEST_ITEM_INDEX: {
				...
				if(source == null || getConsume() == null) {
					actor.sendMessage("Fill the remaining cells.");
					return;
				}
				...
				if(source.getEnchantLevel() >= range[1] || source.getEnchantLevel() < range[0]) {
					actor.sendMessage("This does not fit alkahest.");
					return;
				}
